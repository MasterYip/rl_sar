...
#ifndef EL4090_FSM_HPP
#define EL4090_FSM_HPP

#include "fsm.hpp"
#include "rl_sdk.hpp"

namespace el4090_fsm
{
    ...
    public:
        ...
        void Run() override
        {
            ...
            {
                // 使用config.yaml中配置的预备姿态
                auto pre_running_pos = rl.params.Get<std::vector<float>>("pre_stand_pos");
                if (Interpolate(percent_pre_getup, rl.now_state.motor_state.q, pre_running_pos, 0.5f, "Pre Getting up", true))
                    return;
...
class EL4090FSMFactory : public FSMFactory
{
public:
    EL4090FSMFactory(const std::string &initial) : initial_state_(initial) {}
    std::shared_ptr<FSMState> CreateState(void *context, const std::string &state_name) override
    {
        RL *rl = static_cast<RL *>(context);
        if (state_name == "RLFSMStatePassive")
            return std::make_shared<el4090_fsm::RLFSMStatePassive>(rl);
        else if (state_name == "RLFSMStateGetUp")
            return std::make_shared<el4090_fsm::RLFSMStateGetUp>(rl);
        else if (state_name == "RLFSMStateGetDown")
            return std::make_shared<el4090_fsm::RLFSMStateGetDown>(rl);
        else if (state_name == "RLFSMStateRLLocomotion")
            return std::make_shared<el4090_fsm::RLFSMStateRLLocomotion>(rl);
        return nullptr;
    }
    std::string GetType() const override { return "el_4090"; }
...
REGISTER_FSM_FACTORY(EL4090FSMFactory, "RLFSMStatePassive")

#endif // EL4090_FSM_HPP
